/* ================= BANCO DE DADOS CLÍNICO & VARIÁVEIS ================= */
...
function salvarPaciente() {
    ...
    const registro = {
        ...
        dadosClinicos: dados // Salva o objeto 'dados' completo com notas e labels
    };

    // Pega o banco atual ou cria vazio
    let banco = JSON.parse(localStorage.getItem("db_laudos")) || [];
    
    // Verifica se já existe (update) ou cria novo
    const index = banco.findIndex(p => p.nome.toLowerCase() === nome.toLowerCase());
    if (index >= 0) {
        if(confirm("Paciente já existe. Deseja atualizar os dados?")) {
    ...
    localStorage.setItem("db_laudos", JSON.stringify(banco));
...
function atualizarListaBanco() {
    ...
    // Ordena por nome
    banco.sort((a,b) => a.nome.localeCompare(b.nome));

    banco.forEach((paciente, i) => {
        select.innerHTML += `<option value="${i}">${paciente.nome}</option>`;
    });
}
...
function carregarPacienteDoBanco() {
    ...
    if (p) {
        ...
        // Recupera dados do checklist
        dados = p.dadosClinicos;
        ...
        if (dados.media > 0 && dados.valores.length > 0) {
            ...
            document.getElementById("statusCheck").innerText = "Carregado do Banco. Média: " + dados.media.toFixed(2);
        }
    }
}
...
function excluirPaciente() {
    ...
    if (confirm("Tem certeza que deseja excluir este registro permanentemente?")) {
        let banco = JSON.parse(localStorage.getItem("db_laudos")) || [];
        // Precisamos encontrar o item correto pois o indice do select muda com a ordenação
        const nomeParaExcluir = document.getElementById("listaPacientesSalvos").options[document.getElementById("listaPacientesSalvos").selectedIndex].text;
        
        const novoBanco = banco.filter(p => p.nome !== nomeParaExcluir);
        localStorage.setItem("db_laudos", JSON.stringify(novoBanco));
...
// INICIALIZA O SISTEMA E CARREGA O BANCO
atualizarListaBanco();
atualizarRelatorio();